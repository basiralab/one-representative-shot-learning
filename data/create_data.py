...
for i in range(50):
    edge = np.abs(np.random.normal(mean, std, (116,116))) % 1.0
    mean_s = mean + np.random.rand() % 0.1
    std_s = std + np.random.rand() % 0.1
    
    edge = (edge + edge.T)/2
    
    edge[np.diag_indices_from(edge)] = 0
...
filehandler = open("./classification/edges","wb")
pickle.dump(edges, filehandler)
filehandler.close()

filehandler = open("./classification/labels","wb")
